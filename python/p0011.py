...
This one was tedious but pretty easy. I think I could even make it a generic version of this problem if I could figure
out how to do the type expressions correctly.
...
def cols_by_4(grid: List[List[int]]) -> Iterable[Tuple[int, int, int, int]]:
    ...
    yield from rows_by_4(transposed)


def down_diags_by_4(grid: List[List[int]]) -> Iterable[Tuple[int, int, int, int]]:
...
def up_diags_by_4(grid: List[List[int]]) -> Iterable[Tuple[int, int, int, int]]:
