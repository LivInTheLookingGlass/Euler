...
/**
 * This function takes in a number and returns the British English string
 * that represents it.
 *
...
function toString(n) {
    if (n >= 1000) {
        const thousands = `${toString(0 | (n / 1000 % 100))} thousand`;
        if (n % 1000) {
            return `${thousands} {toString(n % 1000)}`;
        }
        return thousands;
    } else if (n >= 100) {
        const hundreds = `${toString(0 | (n / 100 % 10))} hundred`;
        if (n % 100) {
