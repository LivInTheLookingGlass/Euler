...
contains
    integer(i18t) function p0011() result(answer)
        ...
        do i = 1, 20
            do j = 1, 17
                ! Horizontal section
                tmp = int(grid(i, j), kind=8) * grid(i, j + 1) * grid(i, j + 2) * grid(i, j + 3)
                answer = max(answer, tmp)

                ! Vertical section
                tmp = int(grid(j, i), kind=8) * grid(j + 1, i) * grid(j + 2, i) * grid(j + 3, i)
                answer = max(answer, tmp)
            end do
        end do
        ...
        do i = 1, 17
            do j = 1, 17
                ...
                tmp = int(grid(i, j), kind=8) * grid(i + 1, j + 1) * grid(i + 2, j + 2) * grid(i + 3, j + 3)
                answer = max(answer, tmp)

                ! Left diagonal section
                tmp = int(grid(i, j + 3), kind=8) * grid(i + 1, j + 2) * grid(i + 2, j + 1) * grid(i + 3, j)
                answer = max(answer, tmp)
            end do
        end do
    end function p0011
